...
export function OnboardingScreen({page, sound, onPage, onDone}: {page: number; sound: boolean; onPage: (page: number) => void; onDone: () => void}) {
  if (page < 0) return <View style={styles.fill}><StatusBar hidden /><Image source={backgrounds.onboarding} style={styles.background} resizeMode="cover" /><Video source={media.onboardingVideo} style={styles.background} resizeMode="cover" repeat={false} muted={!sound} onEnd={() => onPage(0)} /><Pressable onPress={() => onPage(0)} style={styles.skip}><Text style={styles.skipText}>Skip</Text></Pressable></View>;
  const item = ONBOARDING[page];
  return <View style={styles.fill}><StatusBar hidden /><Image source={backgrounds.onboarding} style={styles.background} resizeMode="cover" /><SafeAreaView style={styles.content}><View style={styles.card}><Text style={styles.title}>{item.title}</Text><Text style={styles.body}>{item.text}</Text></View><View style={styles.dots}>{ONBOARDING.map((_, i) => <View key={i} style={[styles.dot, i === page && styles.activeDot]} />)}</View><GameButton title={page === 2 ? 'Play' : 'Next'} onPress={() => page === 2 ? onDone() : onPage(page + 1)} /></SafeAreaView></View>;
}

const width = Dimensions.get('window').width;
const styles = StyleSheet.create({
  fill: {flex: 1, backgroundColor: '#087fdc'}, background: {position: 'absolute', width: '100%', height: '100%'}, skip: {position: 'absolute', right: 24, top: 54, backgroundColor: 'rgba(0,0,0,.4)', paddingHorizontal: 18, paddingVertical: 10, borderRadius: 20}, skipText: {color: colors.white, fontWeight: '800'},
